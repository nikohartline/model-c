...
possible_outlier_rows = c(which(Modeling_C$X1 > 300),
                          which(Modeling_C$X1 < -50 & Modeling_C$X4 < -20)) #Labels two outlier rows. Neither has much impact on model assumptions or coefficients herein, but they are kept in the model nonetheless. Uncomment line below to remove them for analyses.
...
summary(y2_logit_model_x1_x2_x3)
anova(y2_logit_model_x1_x2_x3, test = "Chisq")
#This model has a lower AIC value

y1_logit_model_x1_x4 = glm("Y1~X1+X4+X1_sq",
                           family = "binomial",
                           data = Modeling_C,
                           control = list(maxit=100))

summary(y1_logit_model_x1_x4)
anova(y1_logit_model_x1_x4, test = "Chisq")
#AIC value goes up slightly, but model interpretability is better without both correlated predictors in the model (X1 and X3)
