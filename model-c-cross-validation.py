...
poly3 = PolynomialFeatures(3) #create polynomial and interaction features with X1, X2, X3, and X4
X = poly3.fit_transform(Modeling_C[['X1','X2','X3','X4']])[:,1:]
feature_names = poly3.get_feature_names(['X1','X2','X3','X4'])[1:]
...
cv_scores_Y1_X3_X4 = cross_val_score(LR_model, X[['X3','X4']], Y1, cv=10)
...
print("Average accuracy of 10-fold cross-validation for logit model with X3 and X4: %.1f%%" % np.average(cv_scores_Y1_X3_X4*100))
...
LR_model.fit(X[['X1','X4']], Y1)

print("Y1 Model Selected: logit(Y1) = %.3f + (%.3f) * X1 + (%.3f) * X4" % (LR_model.intercept_,LR_model.coef_[0][0],LR_model.coef_[0][1]))
